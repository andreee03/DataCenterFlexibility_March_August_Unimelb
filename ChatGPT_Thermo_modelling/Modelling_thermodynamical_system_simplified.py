...
try:
    ...
    # Changed: Added a deterministic fallback dataset so the model can run even
    # when the external Dataset_treatment.py file is not present in the folder.
    dataset_input = {
        "time_utc": list(range(24)),
        "input_power_IT_room_kW": [85 + 10 * math.sin(2 * math.pi * h / 24) for h in range(24)],
...
def compute_UA(epsilon_calibr, C_min, K):
    ...
    # Changed: Handle the counter-flow Cr ~= 1 limit explicitly to avoid a
    # divide-by-zero in the analytical expression.
...
# ---------------------------------------------------------------------------
# Changed: Use seconds because flow rates are in m3/s and the water-loop slice
# calculation multiplies flow_rate * dt.
DT = 60
NDATA = 1
...
class CoolingTower:
    ...
    def set_leaving_water_temp(self) -> bool:
        ...
        while k < 5 and delta > tolerance:
            ...
            C_air = self.airflow * AIR_SPECIFIC_HEAT * VOLUMIC_MASS_AIR
            # Changed: epsilon_from_UA expects a heat-capacity rate, not a raw airflow.
        ...
        # also should not heat condenser water that is already below that limit.
...
class Chiller_water_cooled:
    ...
    def update_w_out(self) -> bool:
        ...
        # negative load, negative PLR, or unphysical heating when water is already cold.
        ...
        # Changed: Apply the same zero/capacity limits to nominal demand used for power.
...
class Evaporator_loop:
    ...
    def calibration(self, evap_fr, set_p):
        ...
        if self.TES_tank:
            self.Thermal_Energy_Storage_tank(self.TES_tank)
...
class Server_air_cooled:
    ...
    def time_scale(self)-> str:
        ...
        eigenvalues = np.real_if_close(eig(A)[0])
        # Changed: scipy returns complex dtype eigenvalues; convert through the
        # real component after real_if_close so this diagnostic does not fail.
        return f'time1:{-1/float(np.real(eigenvalues[0]))},\n time2 {-1/float(np.real(eigenvalues[1]))}'
...
class DataCenterFacility:
    ...
    def __init__(self ,SERVER_INFO: tuple, CRAC_INFO: tuple, CHILLER_INFO: tuple, EVAP_LOOP_INFO: tuple, COOLING_TOWER_INFO, dt):
        ...
        self.dt = dt
        # Changed: Keep the plotting attribute map on the instance so imported
        # simulations do not depend on a __main__ global variable.
        self.dynamic_attribute_owner = create_correspondance_dico()
    ...
    def update_dict(self, values_dict):
        for key in values_dict:
            owner_name = self.dynamic_attribute_owner[key]
            if owner_name == 'DataCenterFacility':
                owner = self
            elif owner_name in ['CRACUnit', 'Server_air_cooled']:
                # Changed: Use this facility instance instead of the global dc,
                # so plotting works for any DataCenterFacility object.
                owner = getattr(self.ITRoom, owner_name)[0]
            else:
                # Changed: Use this facility instance instead of the global dc.
                owner = getattr(self, owner_name)

            # Changed: Fall back to IT room attributes for aggregate lists like
            # CRACUnit and Server_air_cooled, and do not round non-numeric values.
            if not hasattr(owner, key) and hasattr(self.ITRoom, key):
                owner = self.ITRoom
            value = getattr(owner, key)
            if isinstance(value, (int, float, bool, np.integer, np.floating)):
                values_dict[key].append(round(value, 2))
            else:
...
def retroEngineering_data_center(PUE: float, size_in_kW: float, COP_ref: float,  CC_CRAC_boundaries_kW: tuple, CC_Cool_Tower_boundaries_kW: tuple, dt,  Oversizing = 1, Tier = None)-> DataCenterFacility:
    ...
    COOLING_TOWER_INFO = (Q_cond, n_coolingT_cells, EPSILON_CALIBR, MIN_APPROACH)
...
def MONTE_CARLO(n_simulations, ):
    ...
    # Changed: Generate one random input set per simulation; the old code tried
    # to index single float values and also omitted the required dt argument.
    List_data_centers = [
        retroEngineering_data_center(
            rd.uniform(*PUE_boundaries),
            rd.uniform(*sizes_boundaries_kW),
            rd.uniform(*COP_ref_boundaries),
            CRAC_CC_range,
            Cool_Tower_CC_range,
            DT,
        )
...
def plotting(dict: dict[List]) -> None:
    ...
    if os.environ.get("THERMO_SHOW_PLOTS", "0") != "1":
        # Changed: Skip interactive plot windows by default so repeated command
        # line validation can finish without waiting for a browser/UI.
...
if __name__ == "__main__":
    ...
    dc = retroEngineering_data_center(1.6, 160, 6, CRAC_CC_range, Cool_Tower_CC_range, DT)
