...
def normalize_cooling_type(condenser_type, source_file=None):
    """Return AirCooled or WaterCooled from IDF metadata.

    Changed: Added an explicit normalized cooling type so the final dictionary
    directly tells whether the chiller is air cooled or water cooled.
    """
    text = f"{condenser_type or ''} {source_file or ''}".lower().replace(" ", "").replace("-", "")
    if "aircooled" in text:
        return "AirCooled"
    if "watercooled" in text:
        return "WaterCooled"
    return None
...
def parse_idf_file(idf_path, source_label):
    ...
    while True:
        ...
        rec["cooling_type"] = normalize_cooling_type(rec.get("condenser_type"), source_label)
        rec["is_air_cooled"] = rec["cooling_type"] == "AirCooled"
        rec["is_water_cooled"] = rec["cooling_type"] == "WaterCooled"
