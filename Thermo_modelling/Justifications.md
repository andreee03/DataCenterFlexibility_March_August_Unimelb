...
- Both condensor and evaporator loop use pure water.
- the heat capacity and volumic mass of water are constant and don't depend on temperature.
- Hermetic water-cooled compressor : All compressor work ends up as heat rejected at the condensor
...
\epsilon = \frac{1−exp(−NTU(1−m^∗))}{1−m^∗exp(−NTU(1−m^∗))}​ \\ 

NTU= \frac{K_a*V}{D_{outside, air}} \\ 

m^*(T_{w, in}, T_{w, out}) = \frac{D_{outside, air} C_s(T_{w, in}, T_{w, out})}{D_{cond} C_{p, w}} \\
C_s(T_{w, in}, T_{w, out}) = \frac{h_{sat}(T_{w,in})−h_{sat}(T_{w,out})}{T_{w,in} -T_{w,out}} \cite{Braun}
...
h_{sat}(T)=c_{pa}​T+w_{sat}​(T)[h_{fg}​+c_{pv}​T] #  kJ/kg dry air \\
w_{sat}(T)=0.622 \frac{P_{sat}}{P_{atm}−Psat(T)} \cite{HANDBOOK_ASHRAE}\\
\end{cases}


P_{sat}(T) =0.6112exp(\frac{17.67T}{T+243.5}​) [kPa], T in °C \cite{Magnus} [5] 
...
$ \epsilon \in [0.70 , 0.88]$
$ \epsilon w glycol \in [0.79, 0.89]$

In fact, We compute : 

$\epsilon = \frac{Q_{designed}}{C_{air,rated}(T_{air, in, rated} - T_{w, in, rated}) }$

With, because there is a correlation between Q_{designed} and D_{air,rated}, with
: 
Model 305, 375, 415 \cite{Vertiv_HE} (pages 9-14)
$
Q_{designed} \in [214, 455] kW
D_{air,rated} \in [35, 42] kACFM= [16.5,19.8] m^3 / s

T_{air, in, rated} \in [23.9, 29.4] C
T_{w, in, rated} \in 7.2 C
$


with glycol: \cite{STULZ_HE} (pages 12, 15)
$
Q_{designed} \in [295, 900] kW
D_{air,rated} \in [24, 90] kACFM= [11.3,42.5] m^3 / s

T_{air, in, rated} \in [23.9, 34.9] C
T_{w, in, rated} \in 7.2 C
$

For the link between Q_{designed} and D_{air,rated} is given as a rule of thumb : 350-500 CFM / ton_evap  \cite{CRAC_airflow}
...
$D_{water, rated} \in [8, 24] L/s \cite{Vertex_HE} (pages 9-14)$

$D_{water, rated} \in [6.5, 23] L/s for 6 out of the 10 models,  but extreme bounds are: [3, 48] L/s \cite{STULZ_HE} (pages 12, 15) $ 
